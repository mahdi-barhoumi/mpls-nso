<template>
  <div>
    <div class="card">
      ...
      <Toolbar class="mb-6">
        <template #start>
          <Button label="New" icon="pi pi-plus" severity="Primary" class="mr-2" @click="openNewSiteDialog" />
        ...
        <template #end>
          ...
          <Button label="Refresh" icon="pi pi-refresh" severity="secondary" @click="fetchSites" :loading="loading" />
        </template>
      </Toolbar>

      <!-- DataTable -->
      <DataTable ref="dt" v-model:selection="selectedSites" :value="sites" dataKey="id" :paginator="true" :rows="10"
        :filters="filters"
        paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
        :rowsPerPageOptions="[5, 10, 25]" currentPageReportTemplate="Showing {first} to {last} of {totalRecords} sites"
        selectionMode="multiple" :loading="loading" class="p-datatable-gridlines" stripedRows showGridlines>
        <template #header>
        ...
        <Column field="location" header="Location" sortable style="min-width: 8rem">
          <template #body="slotProps">
            <span class="text-700">{{ slotProps.data.location || 'N/A' }}</span>
          </template>
        </Column>
        <Column field="customer.name" header="Customer" sortable style="min-width: 8rem">
          <template #body="slotProps">
            <span class="text-700">{{ slotProps.data.customer?.name || 'N/A' }}</span>
          </template>
        </Column>
        <Column header="Provider Edge" style="min-width: 10rem">
          <template #body="slotProps">
            <div class="text-700">
              <div class="font-medium">
                {{ slotProps.data.assigned_interface?.router?.hostname || 'N/A' }}
              </div>
              <small class="text-500">via {{ slotProps.data.assigned_interface?.name || 'N/A' }}</small>
            </div>
          </template>
        </Column>
        <Column field="ce_router.hostname" header="Customer Edge" sortable style="min-width: 10rem">
          <template #body="slotProps">
            <div v-if="slotProps.data.ce_router" class="text-700">
              <div class="font-medium">
                {{ slotProps.data.ce_router?.hostname || 'N/A' }}
              </div>
              <small class="text-500">via {{ slotProps.data.ce_router?.interface_name || 'N/A' }}</small>
            </div>
            <div v-else>N/A</div>
          </template>
        </Column>
        <Column header="Routing" style="min-width: 8rem;" headerClass="justify-center flex">
          <template #body="slotProps">
            <Button
              :label="slotProps.data.has_routing ? 'Disable' : 'Enable'"
              :icon="slotProps.data.has_routing ? 'pi pi-ban' : 'pi pi-link'"
              :severity="!slotProps.data.has_routing && !slotProps.data.ce_router ? 'secondary' : (slotProps.data.has_routing ? 'danger' : 'success')"
              variant="outlined"
              size="small"
              class="w-full"
              :loading="routingAction === slotProps.data.id"
              :disabled="!slotProps.data.has_routing && !slotProps.data.ce_router"
              @click="slotProps.data.has_routing ? disableRouting(slotProps.data) : openRoutingPanel($event, slotProps.data)"
              v-tooltip.top="slotProps.data.has_routing
                ? 'Disable Routing'
                : (!slotProps.data.ce_router ? 'No Customer Edge router connected' : 'Enable Routing')"
            />
          </template>
        </Column>
        <Column :exportable="false" style="min-width: 12rem" headerClass="text-center" bodyClass="actions-center">
          <template #body="slotProps">
            <div class="actions-center">
              <Button icon="pi pi-eye" outlined rounded class="mr-2" severity="help"
                @click="viewSite(slotProps.data)" />
              <Button icon="pi pi-pencil" outlined rounded class="mr-2" severity="warn"
                @click="editSite(slotProps.data)" />
              <Button icon="pi pi-trash" outlined rounded severity="danger" :loading="deleting === slotProps.data.id"
    ...
    <OverlayPanel ref="routingPanel">
      <div class="min-w-[320px] max-w-[500px]">
        <div class="font-bold text-lg mb-2">Enable Routing</div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="md:col-span-2">
            <label class="block font-bold mb-1">Protocol</label>
            <Dropdown v-model="routingForm.protocol" :options="['OSPF']" disabled class="w-full" />
          </div>
          <div>
            <label class="block font-bold mb-1">PE Router ID</label>
            <InputText v-model="routingForm.pe_router_id" class="w-full" />
          </div>
          <div>
            <label class="block font-bold mb-1">CE Router ID</label>
            <InputText v-model="routingForm.ce_router_id" class="w-full" />
          </div>
          <div>
            <label class="block font-bold mb-1">Area</label>
            <InputText v-model="routingForm.area" class="w-full" />
          </div>
          <div>
            <label class="block font-bold mb-1">Priority (0-127)</label>
            <InputText v-model="routingForm.priority" type="number" min="0" max="127" class="w-full" />
          </div>
          <div class="md:col-span-2">
            <label class="block font-bold mb-1">Advertised Networks</label>
            <div v-for="(net, idx) in routingForm.advertised_networks" :key="idx" class="flex items-center gap-2 mb-1">
              <InputText v-model="routingForm.advertised_networks[idx]" class="flex-1" />
              <Button icon="pi pi-trash" text size="small" @click="removeAdvertisedNetwork(idx)" :disabled="routingForm.advertised_networks.length === 1" />
            </div>
            <Button icon="pi pi-plus" label="Add Network" text size="small" @click="addAdvertisedNetwork" />
          </div>
          <div class="md:col-span-2">
            <small v-if="routingFormError" class="text-red-500">{{ routingFormError }}</small>
          </div>
        </div>
        <div class="flex justify-end gap-2 mt-4">
          <Button label="Cancel" text @click="routingPanel.hide()" />
          <Button label="Enable" icon="pi pi-check" :loading="routingAction === routingPanelSite?.id" @click="submitRoutingForm" />
        </div>
      </div>
    </OverlayPanel>

    <!-- Site Details Dialog -->
    <Dialog v-model:visible="siteDetailsDialogVisible" :style="{ width: '700px', maxWidth: '95vw' }"
      header="Site Details" :modal="true">
      <div class="flex flex-col gap-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pl-4">
          <div>
            ...
            <div class="mb-2">
              <div>{{ currentSite.customer?.name || '-' }}</div>
              <div v-if="currentSite.customer">
                <small class="text-500">Email: {{ currentSite.customer.email || '-' }}</small><br>
                <small class="text-500">Phone: {{ currentSite.customer.phone_number || '-' }}</small>
              </div>
            </div>
          ...
          <div>
            <div class="font-bold mb-1">Provider Edge:</div>
            <div class="mb-2">
              <span>{{ currentSite.assigned_interface?.router?.hostname || '-' }}</span>
              <small v-if="currentSite.assigned_interface?.name" class="ml-1 text-500">via {{
                currentSite.assigned_interface?.name }}</small>
            </div>
            <div class="font-bold mb-1">Customer Edge:</div>
            <div class="mb-2">
              <span>{{ currentSite.ce_router?.hostname || '-' }}</span>
              <small class="ml-1 text-500">via {{ currentSite.ce_router?.interface_name || '-' }}</small>
            </div>
            <div class="font-bold mb-1">Site VRF:</div>
            <div class="mb-2">
              <span v-if="currentSite.vrf">
                {{ currentSite.vrf.name }} <small class="text-500">(RD: {{ currentSite.vrf.route_distinguisher || '-'
                  }})</small>
              </span>
              <span v-else>-</span>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pl-4">
          ...
          <div>
            <div class="font-bold mb-1">Routing Information:</div>
            <div class="mb-2 flex flex-col">
              <div class="font-bold mt-2">Status:
                <Tag
                :value="currentSite.has_routing ? 'Enabled' : 'Disabled'"
                :severity="currentSite.has_routing ? 'success' : 'danger'"
                class="w-fit mb-2"
              />
              </div>
              <div v-if="currentSite.routing_info">
                <div><b>Protocol:</b> {{ currentSite.routing_info.protocol }}</div>
                <div><b>Priority:</b> {{ currentSite.routing_info.priority ?? '-' }}</div>
                <div>
                  <b>Area{{ Array.isArray(currentSite.routing_info.areas) && currentSite.routing_info.areas.length > 1 ? 's' : '' }}: </b>
                  <span v-if="currentSite.routing_info.areas && currentSite.routing_info.areas.length">
                    {{ currentSite.routing_info.areas.join(', ') }}
                  </span>
                  <span v-else>-</span>
                </div>
                <div>
                  <b>Advertised Networks:</b>
                  <ul v-if="currentSite.routing_info.advertised_networks && currentSite.routing_info.advertised_networks.length">
                    <li v-for="(net, idx) in currentSite.routing_info.advertised_networks" :key="idx">{{ net }}</li>
                  </ul>
                  <span v-else>-</span>
                </div>
              </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pl-4">
          <div>
            ...
            <div class="mb-2">{{ formatDateTime(currentSite.created_at) || '-' }}</div>
          </div>
          <div>
            <div class="font-bold mb-1">Last Updated:</div>
            <div class="mb-2">{{ formatDateTime(currentSite.updated_at) || '-' }}</div>
          </div>
        </div>
      </div>
      <template #footer>
        <Button label="Close" icon="pi pi-times" text @click="siteDetailsDialogVisible = false" />
      </template>
    </Dialog>

    <!-- Site Form Dialog -->
    <Dialog v-model:visible="siteDialogVisible" :appendTo="'body'" :style="{ width: '450px' }"
    ...
    <Dialog v-model:visible="deleteSiteDialog" :style="{ width: '450px' }" header="Confirm" :modal="true">
      <div class="flex items-center gap-4">
        ...
        <span v-if="currentSite">Are you sure you want to delete <b>{{ currentSite.name }}</b>?</span>
      </div>
      <template #footer>
        ...
        <Button label="Yes" icon="pi pi-check" severity="danger" @click="deleteSite"
          :loading="deleting === currentSite?.id" />
      </template>
    </Dialog>

    <!-- Confirm Delete Selected Sites Dialog -->
    <Dialog v-model:visible="deleteSitesDialog" :style="{ width: '450px' }" header="Confirm" :modal="true">
      <div class="flex items-center gap-4">
        <i class="pi pi-exclamation-triangle !text-3xl" />
...
const routingPanelSite = ref(null)
const routingForm = ref({
  protocol: 'OSPF',
  pe_router_id: '1.1.1.1',
  ce_router_id: '2.2.2.2',
  area: '0',
  priority: '',
  advertised_networks: ['0.0.0.0/0'],
})
...
const viewSite = async (site) => {
  try {
    ...
    const response = await SiteService.getSite(site.id)
...
const openRoutingPanel = (event, site) => {
  routingPanelSite.value = site
  routingForm.value = {
    protocol: 'OSPF',
    pe_router_id: '1.1.1.1',
    ce_router_id: '2.2.2.2',
  ...
  routingPanel.value.toggle(event)
}

const addAdvertisedNetwork = () => {
  routingForm.value.advertised_networks.push('')
}
const removeAdvertisedNetwork = (idx) => {
  if (routingForm.value.advertised_networks.length > 1) {
    routingForm.value.advertised_networks.splice(idx, 1)
  }
}

const submitRoutingForm = async () => {
  routingFormError.value = ''
  if (!routingForm.value.protocol) {
    routingFormError.value = 'Protocol is required'
    return
  }
  if (!routingForm.value.pe_router_id || !routingForm.value.ce_router_id) {
    routingFormError.value = 'PE and CE router IDs are required'
    return
  }
  routingAction.value = routingPanelSite.value.id
  try {
    await SiteService.enableRouting(routingPanelSite.value.id, {
      protocol: routingForm.value.protocol,
      pe_router_id: routingForm.value.pe_router_id,
      ce_router_id: routingForm.value.ce_router_id,
      area: routingForm.value.area,
      priority: routingForm.value.priority === '' ? null : Number(routingForm.value.priority),
      advertised_networks: routingForm.value.advertised_networks.filter(n => n.trim() !== ''),
    })
    showMessage(`Routing enabled for "${routingPanelSite.value.name}"`)
    fetchSites()
    routingPanel.value.hide()
  } catch (error) {
    routingFormError.value = 'Failed to enable routing'
  } finally {
    routingAction.value = null
  }
}
